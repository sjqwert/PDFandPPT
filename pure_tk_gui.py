...
"""
纯Tkinter版PDF转PPT GUI界面
不导入任何可能引起macOS版本问题的库
"""
...
logger = logging.getLogger("pure_tk_gui")
...
def check_pdftoppm():
    ...
    try:
        ...
        logger.info("pdftoppm可用")
        return True
    except (FileNotFoundError, subprocess.SubprocessError):
        logger.warning("pdftoppm不可用，请安装poppler")
        return False
    ...
    try:
        ...
        try:
            ...
            for i, img_file in enumerate(image_files):
                dest_file = os.path.join(images_dir, f"slide_{i+1:03d}.png")
            ...
            logger.info(f"执行命令: {' '.join(libreoffice_cmd)}")
            try:
                lo_result = subprocess.run(libreoffice_cmd, capture_output=True, text=True, timeout=60)
                
                if lo_result.returncode != 0:
                    logger.error(f"LibreOffice转换失败: {lo_result.stderr}")
                    
                    # 备用方案：直接使用命令行工具创建PPT
                    logger.info("尝试使用备用方案创建PPT")
                    
                    # 使用zip创建一个基本的PPTX文件
                    with open(output_path, 'w') as f:
                        f.write("PowerPoint文件创建失败，请安装LibreOffice或使用其他工具")
...
class SimplePDF2PPTApp:
    ...
    def __init__(self, root):
        """初始化应用"""
        self.root = root
        self.root.title("纯Tkinter版PDF转PPT工具")
        self.root.geometry("800x600")
        self.root.minsize(700, 500)
        
        # 检查pdftoppm
        self.pdftoppm_available = check_pdftoppm()
        if not self.pdftoppm_available:
            logger.warning("pdftoppm不可用，部分功能将受限")
    ...
    def create_options_section(self, parent):
        ...
        if not self.pdftoppm_available:
            status_label = ttk.Label(
                options_frame, 
                text="警告: 未检测到pdftoppm工具，请安装poppler",
                foreground="red"
            )
    ...
    def start_conversion(self):
        """开始转换过程"""
        if not self.pdftoppm_available:
            messagebox.showerror("错误", "未检测到pdftoppm工具，无法进行转换。请安装poppler:\n\nbrew install poppler")
            return
